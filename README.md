...
- 左侧模型列表底部提供刷新按钮，手动编辑 `config.json` 后无需重启即可重载配置。
- 本地 API 只提供读取、启停、配置重载和日志能力，不开放新增、更新、删除模型配置。
...
`config.json` 为空或不存在时返回空列表 `200`；JSON 损坏时返回 `400` + `config_reload_failed`。
...
| `start_failed` | 启动失败 |
| `restart_failed` | 重启失败 |
| `config_reload_failed` | 配置刷新失败（JSON 损坏或无法读取） |
