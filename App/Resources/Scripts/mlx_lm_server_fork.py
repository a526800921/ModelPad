#!/usr/bin/env python3
"""
mlx_lm.server fork — 原生支持 --kv-bits / --kv-group-size

基于 mlx_lm.server v0.31.3（mlx-lm 0.31.3），改动：
1. 相对导入改为绝对导入（脱离 mlx_lm 包后可独立运行）
2. 新增 --kv-bits / --kv-group-size CLI 参数
3. _serve_single 中将 kv 量化参数传给 stream_generate
...
from mlx_lm.utils import _parse_size, load, sharded_load
...
class ResponseGenerator:
    ...
    def _serve_single(self, request):
        ...
        try:
            ...
            # === [fork] 透传 kv 量化参数到 stream_generate ===
...
def main():
    parser = argparse.ArgumentParser(
        description="MLX Http Server (fork with KV cache quantization)."
    )
    parser.add_argument(
        "--model",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Number of bits for KV cache quantization (default: None, disabled)",
    )
    parser.add_argument(
        "--kv-group-size",
        type=int,
    ...
    if args.kv_bits is not None:
        logging.info(f"KV cache quantization enabled: bits={args.kv_bits}, group_size={args.kv_group_size}")
